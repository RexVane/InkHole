...
- **应用内主界面**：深色双栏工作台——稳定的发送区、设备列表、最近接收、应用内设置页和自适应长文本；设备名/口令/收件箱/自启等设置集中管理，桌宠挂件保留为可开关选项
...
- **检查更新 / 应用内更新**：设置页显示当前版本并可一键检查 GitHub 最新版；Android 更新弹窗同时显示新版本状态与简洁变化摘要；Windows 打包版可直接在应用内下载覆盖并自动重启，Android 可在设置内下载安装新 APK，无需手动去 Releases 下载再删旧版
...
提 [Issue](https://github.com/RexVane/InkHole/issues) 时请附上：操作系统与版本、应用版本（如 v1.3.15）、网络环境（家庭 WiFi / 公司网络 / 热点）、具体报错信息或截图。
...
覆盖三块：**P2P 引擎端到端**（TCP 直连传输、端到端加密 WHE1/WHE2、设备选择切换、对端离线、多文件队列、路径穿越防御、半截文件不落盘、恶意 size 拒收、同名设备共存、幽灵设备探测剔除、虚拟网卡过滤、目录打包往返等）、**手动添加设备**（启动注册、真实直连传输、离线剔除与回线自动恢复、配置增删同步）、**桌面主窗口离屏冒烟**（bridge 契约、设置页、手动设备 UI 往返、IP 输入纠正/分段、更新版本比较）。40 项全通过。
