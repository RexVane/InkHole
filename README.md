...
两台设备连同一个 WiFi，各开一个墨洞，mDNS 自动发现彼此，拖文件进去就 TCP 直连传过去。支持端到端加密（大文件分块流式）、传输回执与进度、手动选目标设备、开机自启。自动发现不可用的网络里，桌面端可在设置中手动添加对方 IP 与端口直连。Windows / macOS / Android 三平台互通。
