...
- **手动添加设备（跨网络直连）**：自动发现不可用时，在设置里填对方 IP 与监听端口直连；设备离线自动移出列表、回线自动恢复。配合 Tailscale 等虚拟局域网即可跨网传输（见下文）
...
提 [Issue](https://github.com/RexVane/InkHole/issues) 时请附上：操作系统与版本、应用版本（如 v1.2.0）、网络环境（家庭 WiFi / 公司网络 / 热点）、具体报错信息或截图。
...
覆盖三块：**P2P 引擎端到端**（TCP 直连传输、端到端加密 WHE1/WHE2、设备选择切换、对端离线、多文件队列、路径穿越防御、半截文件不落盘、恶意 size 拒收、同名设备共存、幽灵设备探测剔除、虚拟网卡过滤、目录打包往返等）、**手动添加设备**（启动注册、真实直连传输、离线剔除与回线自动恢复、配置增删同步）、**桌面主窗口离屏冒烟**（bridge 契约、设置页、手动设备 UI 往返）。37 项全通过。
