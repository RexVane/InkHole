...
墨洞 Android 客户端（当前版本 `1.5.1`），与 Windows/macOS 桌面端通过局域网、Tailscale、一次性短码或 SSH VPS 中继互传文件。
...
- 设置页顶部分行显示本机、版本和端口，并按设备设置、存储、传输安全、跨网络配置分组；跨网络配置包含 Tailscale、一次性短码和 SSH 中继三个页签，帮助与更新是固定的单一公共区域，不随页签重复创建。
