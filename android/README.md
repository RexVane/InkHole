...
墨洞 Android 客户端（当前版本 `1.5.4`），与 Windows/macOS 桌面端通过局域网、Tailscale、一次性短码或 SSH VPS 中继互传文件。

## 功能

- 使用 Android NSD 发现 `_inkhole._tcp` 服务并手动选择发送目标；支持普通 Wi-Fi 和 Android 手机提供热点两种局域网拓扑。
