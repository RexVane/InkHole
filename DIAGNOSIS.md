...
1. **全部流量双向经过单一 VPS 中继，没有任何 P2P 打洞**（无 STUN/ICE/QUIC；wormhole 短码模式两端在 NAT 后也基本落到 transit relay）。速度上限 = min(两端到 VPS 的国际带宽)，中国大陆 ↔ 新加坡晚高峰常只有几 Mbps。**这是物理瓶颈，参数调优救不了。**
...
1. **跨网提速的根本解：P2P 打洞，VPS 退居信令+兜底**。用 QUIC（quic-go）+ UDP 打洞：两端通过 VPS 交换地址（信令极轻），先尝试直连（多数家用 NAT 可打通），打不通再回落 VPS 中继。直连成功时速度只受两端宽带限制，与 VPS 无关。
