...
func (c *Client) Connect(ctx context.Context) (*ConnectInfo, error) {
	...
	if c.httpClient != nil {
		options = &websocket.DialOptions{HTTPClient: c.httpClient}
	}
	c.wsClient, _, err = websocket.Dial(ctx, c.url, options)
	if err != nil {
	...
	// The context used to establish the WebSocket may be a short startup
	// deadline. Mailbox reads must outlive that deadline until Close is called.
	c.readCtx, c.readCancel = context.WithCancel(context.Background())
	go c.readMessages(c.readCtx)
