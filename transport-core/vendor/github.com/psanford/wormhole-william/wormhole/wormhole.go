...
type Client struct {
	...
	// ProxyURL optionally routes the rendezvous WebSocket and transit relay
	// through an HTTP or HTTPS CONNECT proxy. Direct peer hints stay direct.
	ProxyURL string
...
var (
	...
	DefaultRendezvousURL = "wss://relay.magic-wormhole.io/v1"
...
func (c *Client) rendezvousOptions() ([]rendezvous.ClientOption, error) {
	if c.ProxyURL == "" {
		return nil, nil
	}
	...
	return []rendezvous.ClientOption{
		rendezvous.WithHTTPClient(&http.Client{Transport: transport}),
	}, nil
}
