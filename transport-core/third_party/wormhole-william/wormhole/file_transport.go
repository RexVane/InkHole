...
func newFileTransport(transitKey []byte, appID, relayAddr string, proxyURLs ...string) *fileTransport {
	...
	if len(proxyURLs) > 0 {
		transport.proxyURL = proxyURLs[0]
	}
	return transport
}
...
func (t *fileTransport) connectToRelay(ctx context.Context, addr string, successChan chan net.Conn, failChan chan string) {
	conn, err := dialTransitRelay(ctx, t.proxyURL, addr)
	if err != nil {
		failChan <- addr
		return
	}
...
func (t *fileTransport) listenRelay(contexts ...context.Context) error {
	if t.relayAddr == "" {
		return nil
	}
	...
	conn, err := dialTransitRelay(ctx, t.proxyURL, t.relayAddr)
	if err != nil {
		return err
	}
...
func dialTransitRelay(ctx context.Context, proxyAddress, target string) (net.Conn, error) {
	if proxyAddress == "" {
	...
	if proxyURL.Port() == "" {
		...
		proxyHost = net.JoinHostPort(proxyURL.Hostname(), port)
	}
	raw, err := (&net.Dialer{}).DialContext(ctx, "tcp", proxyHost)
	if err != nil {
	...
	if proxyURL.Scheme == "https" {
		...
		if err := secure.HandshakeContext(ctx); err != nil {
			return closeWithError(fmt.Errorf("secure proxy handshake: %w", err))
		}
		conn = secure
	}
	...
	if err := request.Write(conn); err != nil {
		return closeWithError(fmt.Errorf("write proxy CONNECT: %w", err))
	}
	response, err := http.ReadResponse(bufio.NewReader(conn), request)
	if err != nil {
		return closeWithError(fmt.Errorf("read proxy CONNECT: %w", err))
	}
	if response.StatusCode != http.StatusOK {
		return closeWithError(fmt.Errorf("proxy CONNECT returned %s", response.Status))
	}
	return conn, nil
}
