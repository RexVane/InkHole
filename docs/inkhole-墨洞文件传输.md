...
tests/test_p2p.py       P2P 引擎端到端测试(18 组 70 项)
...
覆盖：TCP 直连传输、端到端加密（WHE1/WHE2 分块）、设备选择切换、对端离线（不自动切换）、回调触发、多文件连续发送、路径穿越防御、半截文件不落盘、恶意 size 拒收、同名设备共存与精确离线、口令不一致 ACK 失败、传输进度回调、分块加密往返、分块流篡改/重排检测、发送队列、仅接收目标设备、多地址回退、幽灵设备探测剔除。**18 组 70 项全通过**（也兼容 `pytest`）。
