...
覆盖三块：**P2P 引擎端到端**（TCP 直连传输、端到端加密 WHE1/WHE2 分块、设备选择切换、对端离线不自动切换、回调触发、多文件连续发送、路径穿越防御、半截文件不落盘、恶意 size 拒收、同名设备共存与精确离线、口令不一致 ACK 失败、传输进度回调、分块流篡改/重排检测、发送队列、仅接收目标设备、多地址回退、幽灵设备探测剔除、桌面同名不覆盖、文件名清洗、目录打包、文件夹发送）；**手动添加设备**（启动注册与真实直连传输、离线剔除与回线自动恢复、配置增删同步）；**桌面主窗口离屏冒烟**（bridge 契约、设置页、手动设备 UI 往返、非法输入拒绝、IP 输入纠正与分段、更新版本比较）。**40 项全通过**。
