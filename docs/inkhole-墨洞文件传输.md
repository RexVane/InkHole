...
一个"无障碍"文件互传通道。两台电脑各放一个墨洞桌宠（黑洞造型、乳白色吸积盘、桌面小图标大小），连同一个 WiFi/路由器即可。mDNS 自动发现对端，拖文件进去就 TCP 直连传过去——不经过任何中转服务器。

体验：**右键选目标 → 拖进去 → 黑洞收缩吸入 → 对端黑洞膨胀吐出 → 文件落在收件箱**。

## 原理与架构

墨洞分为两层：底层是 P2P 引擎（mDNS 发现 + TCP 直连），上层是桌宠挂件（动画 + 拖拽）。
...
- **mDNS 发现**：每个墨洞启动后注册 `_inkhole._tcp.local.` 服务（zeroconf），同时浏览局域网内的同名服务。发现对端后自动加入设备列表，由用户右键手动选择发送目标。
...
**支持什么格式？** 全部。底层是 TCP 二进制流，逐字节搬运，不关心内容。支持拖文件夹（桌面端自动递归打包成 zip 发送）；同名文件自动加 " (2)" 后缀不覆盖。
...
覆盖：TCP 直连传输、端到端加密（WHE1/WHE2 分块）、设备选择切换、对端离线（不自动切换）、回调触发、多文件连续发送、路径穿越防御、半截文件不落盘、恶意 size 拒收、同名设备共存与精确离线、口令不一致 ACK 失败、传输进度回调、分块加密往返、分块流篡改/重排检测、发送队列、仅接收目标设备、多地址回退、幽灵设备探测剔除、同名不覆盖、文件名清洗、目录打包、文件夹发送。**29 组 123 项全通过**（也兼容 `pytest`）。
