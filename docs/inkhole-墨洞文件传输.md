...
2. 点击「选择文件」或把文件/文件夹拖入窗口；文件夹会先打包为同名 zip。
...
在主窗口「设置」中点击收件箱旁的「浏览」即可修改。也可以通过命令行覆盖：
...
**支持什么格式？** 全部。底层是 TCP 二进制流，逐字节搬运，不关心内容。支持拖文件夹（桌面端自动递归打包成 zip 发送）；Python 桌面接收端遇到同名文件会自动加 " (2)" 后缀。
...
覆盖：TCP 直连传输、端到端加密（WHE1/WHE2 分块）、设备选择切换、对端离线（不自动切换）、回调触发、多文件连续发送、路径穿越防御、半截文件不落盘、恶意 size 拒收、同名设备共存与精确离线、口令不一致 ACK 失败、传输进度回调、分块加密往返、分块流篡改/重排检测、发送队列、仅接收目标设备、多地址回退、幽灵设备探测剔除、桌面同名不覆盖、文件名清洗、目录打包、文件夹发送。**29 组 123 项全通过**（也兼容 `pytest`）。
